...
module.exports = {
   ...
   * @返回: true 表示一致; false 表示不同 reject 返回错误原因
   */
  ...
  findSelect(modelName, select, isShow_id) {
    return new Promise(async function (resolve, reject) {
      ...
      // 选择要查询的字段
      model.find({}, show_id).select(select)
        .then(result => {
          if (result) { resolve(result) }
          else { reject('没有查询到任何信息') }
  ...
  findByOptions(modelName, rule, options) {
    return new Promise(async function (resolve, reject) {
      try {
        ...
        // 分页查询的方式
        if (options.last_id) {
          data = await model.find({ '_id': { "$lt": options.last_id } })
            .limit(parseInt(options.find_size))
            .sort({ '_id': -1 })
        } else {
          data = await model.find(rule)
  ...
  findAndOptionsToSort(modelName, rule, options, sort) {
    return new Promise(async function (resolve, reject) {
      try {
        if (!(options.find_size && options.page_No)) { return reject('分页查询不满足条件') }
        // 从模型库中查找是否有对象
        let model = await mongo.getModel(modelName)
        // 获得数据总量
        const count = await model.countDocuments(rule)
